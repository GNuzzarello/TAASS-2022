...
public class CartListAdapter extends RecyclerView.Adapter<CartListAdapter.ViewHolder> {
    private ArrayList<TicketOrder> plist;
    private ManagementCart managementCart;
    private ChangeNumberItemsListener changeNumberItemsListener;

    public CartListAdapter(ArrayList<TicketOrder> plist, Context context, ChangeNumberItemsListener changeNumberItemsListener) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.title.setText(plist.get(position).getTicket().getName());
        holder.feeEachItem.setText(String.valueOf(plist.get(position).getTicket().getPrice()));
        holder.totalEachItem.setText(String.valueOf(Math.round((plist.get(position).getNum() * plist.get(position).getTicket().getPrice()) * 100.0) / 100.0));
        ...
        String pic_Url = plist.get(position).getTicket().getPictureUrl();
